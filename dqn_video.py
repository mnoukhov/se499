...
register(
    id='Video-v0',
    entry_point='gym.envs.atari:VideoEnv',
    timestep_limit=2000,
    reward_threshold=2000.0,
    kwargs={
        'videos_folder': '/home/michael/Documents/se499/vot2016',
    },
)

args.env_name = 'Video-v0'
...
# is lost.
def _step(a):
    move_y, move_x = env._action_set[a]
    reward = 0.0
    frames = []

    for _ in range(WINDOW_LENGTH):
        env.bounding_box.move(move_x, move_y)
        reward = env.target.iou(env.bounding_box)
        env.frame, env.target = env.videos_input.get_next_frame()
